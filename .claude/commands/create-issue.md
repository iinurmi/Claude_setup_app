...
Save the issue as a local markdown file at `issues/<YYYY-MM-DD>-<slug>.md` (e.g. `issues/2026-03-01-fix-auth-redirect.md`).
Create the `issues/` directory if it doesn't exist. Do not open or explain the file after saving — just confirm the filename.

> Note: Linear/Jira integration will be added later. For now, local files are the source of truth.
